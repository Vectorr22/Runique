...
class SyncRunWorkerScheduler(
    ...
    private val syncDao: RunPendingSyncDao,
    ...
    private suspend fun scheduleDeleteRunWorker(runId: RunId) {
        ...
        syncDao.upsertDeletedRunSyncEntity(entity)
    ...
    private suspend fun scheduleFetchRunsWorker(interval: Duration) {
        ...
        val workRequest = PeriodicWorkRequestBuilder<FetchRunsWorker>(interval.toJavaDuration())
            .setConstraints(
                Constraints.Builder()
                    .setRequiredNetworkType(NetworkType.CONNECTED)
                    .build()
